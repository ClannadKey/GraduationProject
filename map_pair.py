...
def map_pair(refined_time_traj,refined_lon_traj,refined_lat_traj):
    L=refined_time_traj.shape[0]
    for i in range(L-1):
        start_time=refined_time_traj[i]
        start_lon=refined_lon_traj[i]
        start_lat=refined_lat_traj[i]
        #end_time=refined_time_traj[i+1]
        end_lon=refined_lon_traj[i+1]
        end_lat=refined_lat_traj[i+1]
        if (0<=start_time and start_time<time_split) and (np.abs(start_lon-end_lon)+np.abs(start_lat-end_lat)>0):
            if (0<=start_lon and start_lon<lon_split) and (0<=start_lat and start_lat<lat_split):
                start_location=M[start_lon,start_lat]
            else:
                start_location=N
            if (0<=end_lon and end_lon<lon_split) and (0<=end_lat and end_lat<lat_split):
                end_location=M[end_lon,end_lat]
            else:
                end_location=N
            print(str(start_time)+'S'+str(start_location)+'S'+str(end_location)+'\t1')
        else:
            pass
...
def process_trajectories(line):
    ...
    for cutted_time_traj,cutted_lon_traj,cutted_lat_traj in zip(cutted_time,cutted_lon,cutted_lat):
        ...
        map_pair(refined_time_traj,refined_lon_traj,refined_lat_traj)
